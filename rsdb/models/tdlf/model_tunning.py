...
from tdlf.latent_factor import LatentFactorModel
from tdlf.temporal_static import TemporalStaticModel
from tdlf.temporal_dynamic import TemporalDynamicModel
...
def latent_factor_tunning(data: pd.DataFrame):
    ...
    def build_model_latent_factor(hp):
        ...
        )  # I use legacy version of Adam, change it when needed
        return model
    ...
    # start seaching
    tuner.search(
        train_data, epochs=150, validation_data=test_data, callbacks=[early_stopping]
    )
    ...
    model.fit(
        train_data, epochs=500, validation_data=test_data, callbacks=[early_stopping]
    )
...
def temporal_data_split(data: pd.DataFrame) -> pd.DataFrame:
    ...
    train_df["review_time(unix)"] = (
        train_df["review_time(unix)"] - time_mean
    ) / time_std
...
def temporal_static_tunning(data: pd.DataFrame):
    ...
    data = data[
        ["reviewer_id", "gmap_id", "time", "time_bin", "user_mean_time", "rating"]
    ]
